...
def test_classify_pain_point_recognizes_a_real_pain_point(adapter: ClaudeLLMSearchAdapter) -> None:
    item = _make_item(
        ...
        "alerted when this happens. Anyone found a workaround?"
    )
...
def test_judge_solvable_accepts_a_solo_dev_buildable_problem(adapter: ClaudeLLMSearchAdapter) -> None:
    pain_points = [
        PainPoint(
            id=str(uuid.uuid4()),
            raw_item=_make_item("I wish there was a tool to detect when an LLM API's output format changes."),
            summary="No tool detects silent LLM output-format drift in automation pipelines.",
            created_at=datetime(2026, 7, 12, 12, 0, 0),
        )
    ]
...
def test_judge_solvable_rejects_a_platform_only_problem(adapter: ClaudeLLMSearchAdapter) -> None:
    pain_points = [
        PainPoint(
            id=str(uuid.uuid4()),
            raw_item=_make_item("OpenAI's API has been down for everyone all morning, nothing works."),
            summary="A live OpenAI API outage — only OpenAI can fix this.",
            created_at=datetime(2026, 7, 12, 12, 0, 0),
        )
    ]
    judgement = adapter.judge_solvable(pain_points)
...
def test_write_brief_narrative_produces_a_summary_and_sketch(adapter: ClaudeLLMSearchAdapter) -> None:
    pain_points = [
        PainPoint(
            id=str(uuid.uuid4()),
            raw_item=_make_item("LLM output format drift keeps breaking my automation."),
            summary="No tool detects silent LLM output-format drift in automation pipelines.",
            created_at=datetime(2026, 7, 12, 12, 0, 0),
        )
    ]
